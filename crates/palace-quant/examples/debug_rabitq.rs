use palace_quant::rabitq::{RaBitQIndex, RaBitQCode};
use rand::Rng;
...
fn main() {
    ...
    // Standard L2
    let sqr_dist = v1.iter().zip(v2.iter())
        .map(|(a, b)| (a - b) * (a - b))
        .sum::<f32>();
    ...
    println!("RaBitQ Error: {:.2}%", (est_dist - sqr_dist).abs() / sqr_dist * 100.0);
    ...
    let ip = q_res.iter().zip(v_res.iter()).map(|(a, b)| a * b).sum::<f32>();
