...
fn bench_compact_memory() {
    ...
    println!(
        "  │  Upper-layer:    {:>6}  ({:>4.1}% of nodes)           │",
        upper_pre,
        upper_pre as f64 / total_pre as f64 * 100.0
    );
    println!(
        "  │  Layer-0 nodes:  {:>6}  ({:>4.1}% of nodes)           │",
        layer0_pre,
        layer0_pre as f64 / total_pre as f64 * 100.0
    );
    println!(
        "  │  Float vec RAM:  {:>7.1} KB (actual, Vec::capacity)  │",
        vec_bytes_pre as f64 / 1024.0
    );
    println!(
        "  │  Total estimated:{:>7.1} KB                          │",
        total_est_pre as f64 / 1024.0
    );
    println!("  └───────────────────────────────────────────────────────┘");
    ...
    println!("  ├───────────────────────────────────────────────────────┤");
    println!(
        "  │  Total nodes:    {:>6}  (unchanged)                 │",
        total_post
    );
    println!(
        "  │  Nodes with vec: {:>6}  (upper-layer only)          │",
        upper_post
    );
    println!(
        "  │  Float vec RAM:  {:>7.1} KB  (actual post-compact)  │",
        vec_bytes_post as f64 / 1024.0
    );
    println!(
        "  │  Freed float:    {:>7.1} KB  ({:.1}% of pre-compact) │",
        freed_bytes as f64 / 1024.0,
        if vec_bytes_pre > 0 {
            freed_bytes as f64 / vec_bytes_pre as f64 * 100.0
        } else {
            0.0
        }
    );
    ...
    for &ef in &[32usize, 64, 128] {
        ...
        for (i, q) in queries.iter().enumerate() {
            ...
            if !results.is_empty() && gt[i].contains(&results[0]) {
                recall_1 += 1.0;
            }
        }
        let qps = n_queries as f64 / start.elapsed().as_secs_f64();
        let status = if (recall_10 / n_queries as f64) < 0.10 {
            "✗ COLLAPSED"
        } else if (recall_10 / n_queries as f64) < 0.50 {
            "⚠ DEGRADED"
        } else {
            "✓ OK"
        };
        println!(
            "  ef={:<3} │ R@1={:>5.1}%  R@10={:>5.1}%  QPS={:>7.0}  {}",
            ef,
            recall_1 / n_queries as f64 * 100.0,
            recall_10 / n_queries as f64 * 100.0,
            qps,
            status
        );
    }
    ...
    if reduction_vs_classic >= 30.0 {
        println!(
            "\n  ✓ MEMORY CLAIM VERIFIED: {:.1}% reduction vs classic HNSW (claimed: 38%)",
            reduction_vs_classic
        );
    } else {
        println!(
            "\n  ⚠ MEMORY CLAIM PARTIAL: {:.1}% vs classic HNSW (claimed: 38%)",
            reduction_vs_classic
        );
    }
}
